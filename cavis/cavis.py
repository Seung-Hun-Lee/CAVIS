...
from .criterion_cavis import VideoSetCriterion, loss_reid
...
from .video_cavis_modules import TemporalRefiner, CAVIS_Tracker
...
class CAVIS_segmenter(MinVIS):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            # Prototypical Cross-frame Contrastive Loss (See Sec. 4.2) 
            prototypical_loss = self.get_prototypical_contrastive_loss(outputs, targets)
            outputs, targets = self.frame_decoder_loss_reshape(outputs, targets)
            
            # It contains L_ctx (See Eq. (8) in Sec. 4.1.1)
            losses = self.criterion(outputs, targets)
            
            losses.update(prototypical_loss)
...
@META_ARCH_REGISTRY.register()
class CAVIS_offline(CAVIS_online):
    """
    Training temporal refiner proposed in DVIS.
    """
    @configurable
    def __init__(
        self,
    ...
    def segmentor_windows_inference(self, images_tensor, window_size=5):
        ...
        for i in range(iters):
            ...
            del features['res2'], features['res3'], features['res4'], features['res5']
            del out['pred_masks'], out['pred_reid_embed']
            for j in range(len(out['aux_outputs'])):
                del out['aux_outputs'][j]['pred_masks'], out['aux_outputs'][j]['pred_logits']
